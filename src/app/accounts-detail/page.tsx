...
export default function AccountsDetailPage() {
  ...
  const getAccountColor = (accountType: string) => {
    const colorMap: { [key: string]: string } = {
      ...
      salaries_wages: 'bg-yellow-50 border-yellow-200 text-yellow-800',
      taxes_permits_licenses: 'bg-cyan-50 border-cyan-200 text-cyan-800'
    };
            ...
            <div className="gradient-card rounded-2xl p-8 elevated-box">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-600">Complete view of all account entries across all types</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center">
                  <span className="text-white text-xl">📊</span>
                </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {Object.entries(accountsData.accounts).flatMap(([accountKey, account]) => 
                      getFilteredEntries(account.entries).map((entry, index) => (
                        <tr key={`${accountKey}-${entry.id}`} className="hover:bg-gray-50 transition-colors duration-200">
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-600">
                            {index + 1}
                          </td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm">
                            <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getAccountColor(accountKey)}`}>
                              {account.name}
                            </span>
                          </td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-700">
